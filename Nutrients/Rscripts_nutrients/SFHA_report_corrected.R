...
#packages
lapply(c("CDECRetrieve","tidyverse", "lubridate", "viridis", "ggpubr", "janitor"), require, character.only = TRUE)

#set directory
setwd("C:/Users/jtorre/Desktop/Github_Repos/ND-Annual/Nutrients/Rscripts_nutrients")
...
physical_wq <- read_csv("C:/Users/jtorre/Desktop/Github_Repos/ND-Annual/Nutrients/Data_nutrients/NDFS_WQ_Data_2021.csv", na = c("", "N/A", "NA"), skip = 1) %>%#need to write pre-cleaning steps into this script
  clean_names()
#data cleaning: filter to NDFA, 2020 samples, change to long format for wq measurements
physical_wq2 <- physical_wq %>%
  filter(measuring_program_name %in% c("NDFA", "NDFS", "Shared")) %>% #filter for NDFS data
